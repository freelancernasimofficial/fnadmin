...
export default function SideMenu({ className, data }: SideMenuProps) {
  return (
    <div
      className={`w-64 z-15 transition-transform top-12  dark:bg-gray-900 h-screen bg-white fixed  ${className ?? ''}`}
    >
      <ul className=" font-medium py-5  [&_li]:px-3 [&_li_a]:px-3 [&_li_a]:h-11 [&_li_a]:flex [&_li_a]:rounded-lg [&_li_a]:items-center">
        {data?.map((item, index) => {
          return item.dropdownLinks?.length ? (
            ...
            >
              ...
              >
                ...
                <span className="ml-3 menuItemText text-gray-950 dark:text-white">{item.title}</span>
              </Link>
                  ...
                  return (
                    <li tabIndex={dIndex} key={dIndex} className="group/dItem">
                      ...
                      >
                        ...
                        <span className="ml-3 menuItemText text-gray-950 dark:text-white">{dItem.title}</span>
                      </Link>
                    </li>
                  )
                })}
